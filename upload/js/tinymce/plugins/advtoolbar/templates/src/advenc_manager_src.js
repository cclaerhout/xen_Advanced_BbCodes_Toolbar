...
{    
	XenForo.Adv_Template_Enc = 
	{
		AjaxResponse : false,
		init : function($element) 
		{
			var t = XenForo.Adv_Template_Enc, ed, content;
			...
			//SKIN OPTIONS
			$element.find('#skins li').click(function(e)
			{
				$target = $element.find('#ctrl_skin');
				$active = $element.find('#skins li.active');
			...
			//FLOAT OPTIONS
			$element.find('#float li').click(function(e)
			{
				$target = $element.find('#ctrl_float');
				$active = $element.find('#float li.active');
		...
		html2bbcode : function(ajaxdata)
		{
			...
			var t = XenForo.Adv_Template_Enc;
	 ...
	 XenForo.register('#adv_enc', 'XenForo.Adv_Template_Enc.init');
}
